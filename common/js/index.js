$(document).ready(function() {
	$(".tab span").click(function(){
		...
		$(".tab_wrapper .tab_list").hide();
		$(".tab_wrapper .tab_list").eq($(".tab span").index(this)).show().siblings().hide();	
	})
	...
	//加载设备信息
	selectdeveic();
	setInterval(selectdeveic,15000);
	//获取最新设备信息（设备表联动设备记录表）
		...
		$.ajax({
			...
			success: function(data) {
				...
				for(var i = 0; i < 16; i++) {
					...
					/*
					//温度报警
					if(datas[6] > (parseFloat(datas[8])+5) || datas[6] < (parseFloat(datas[8])-5)){
						$('#YT'+(i+1)+'Re').addClass("YTRe_alarm");
						tem_count++;
					}else{
						$('#YT'+(i+1)+'Re').removeClass("YTRe_alarm");
					}
					*/

					//温度显示
					$('.tem'+(i+1)).html('</br>温度：'+parseFloat(datas[6]).toFixed(1)+'℃');
					
					
					if(parseFloat(datas[15]) < 5){
						$('#CYT'+(i+1)+'Re').addClass("YTRe_stop");
					}else{
						$('#CYT'+(i+1)+'Re').removeClass("YTRe_stop");
					}
					//风机速度反馈和风机速度相减的绝对值大于等于5故障
					var alarm1 = Math.abs(parseFloat(datas[15]) - parseFloat(datas[14]));
					if(alarm1 >= 5){
						$('#CYT'+(i+1)+'Re').addClass("YTRe_alarm");
						air_count++;
					}else{
						$('#CYT'+(i+1)+'Re').removeClass("YTRe_alarm");
					}

					//送风温度
					$('.sf'+(i+1)).html('</br>送风温度：'+ parseFloat(datas[0]).toFixed(1) + "°C");
					...
					//送风温度
					li_list.eq(i).find(".indoortem").html(parseFloat(datas[6]).toFixed(1) + "℃");
					li_list.eq(i).find(".suptem").html(parseFloat(datas[0]).toFixed(1) + "℃");
				}
				$('.status_run').html(status_count);
				$('.status_stop').html(16 - status_count);
				$('.alarm_run').html(alarm_count);
				
				if(alarm_count > 0 || tem_count > 0 || air_count > 0) {
					//console.log($('#audio'));
					//console.log($('#audio')[0]);
					//console.log($('#audio').get(0));
					var audio = $('#audio')[0];
					audio.play();//dom对象才有play方法
				}
				...
				for(var j = 16; j < 25; j++) {
					...
					if(time === '0') {
						...
						$('.status' + (j - 15)).html('</br>影厅：空闲');
					} else {
						li_list.eq(j - 16).find(".hallstatus").html('放映');
						$('.status' + (j - 15)).html('</br>影厅：放映');
					}
					// 10分钟
